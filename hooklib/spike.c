...
static void spike_fn_amd_error(const char* module, int line_no, const char *fmt, ...)
{
    char line[40960];
    va_list ap;
...
static void spike_insert_log_levels_d(ptrdiff_t rva, size_t count)
{
    uint32_t *levels;
...
static void spike_insert_log_levels_d1(ptrdiff_t rva, size_t count)
{
    uint8_t *base;
    uint32_t *levels;
...
static void spike_hook_read_config(const wchar_t *spike_file)
{
    ...
    for (;1;) {
        ...
        match = sscanf(line, "memwrite %i %i", &rva, &count);

        if (match == 2) {
            dprintf("memwrite %d\n", rva);
            spike_insert_log_levels_d((ptrdiff_t) rva, count);
        }

        match = sscanf(line, "memwrite_offset %i %i", &rva, &count);

        if (match == 2) {
            dprintf("memwrite_offset %d\n", rva);
            spike_insert_log_levels_d1((ptrdiff_t) rva, count);
        }
        ...
        match = sscanf(line, "j_amd_error %i", &rva);

        if (match == 1) {
            spike_insert_jmp((ptrdiff_t) rva, spike_fn_amd_error);
        }
