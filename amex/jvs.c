...
static HRESULT jvs_handle_open(struct irp *irp, bool already_checked_fd);
...
static HRESULT jvs_handle_irp(struct irp *irp)
{
    ...
    switch (irp->op) {
    case IRP_OP_OPEN:   return jvs_handle_open(irp, false);
...
HRESULT jvs_handle_irp_ex(struct irp *irp){
    assert(irp != NULL);
    ...
    switch (irp->op) {
    case IRP_OP_OPEN:   return jvs_handle_open(irp, true);
    case IRP_OP_CLOSE:  return jvs_handle_close(irp);
    case IRP_OP_IOCTL:  return jvs_handle_ioctl(irp);
    default:            return HRESULT_FROM_WIN32(ERROR_INVALID_FUNCTION);
    }
...
static HRESULT jvs_handle_open(struct irp *irp, bool already_checked_fd)
{
    ...
    if (!already_checked_fd && !wstr_eq(irp->open_filename, L"$jvs")) {
        return iohook_invoke_next(irp);
    }
