...
static HRESULT led1509306_req_enter_bootloader(int board, const struct led1509306_req_any *req);
...
void led1509306_config_load(struct led1509306_config *cfg, const wchar_t *filename)
{
    ...
    cfg->fw_ver = GetPrivateProfileIntW(L"led", L"fw_ver", 0x90, filename);
    cfg->fw_sum = GetPrivateProfileIntW(L"led", L"fw_sum", 0xadf7, filename);
    cfg->port1 = GetPrivateProfileIntW(L"led", L"port1", 11, filename);
    cfg->port2 = GetPrivateProfileIntW(L"led", L"port2", 17, filename);

    GetPrivateProfileStringW(L"led", L"board_number", L"15093-06", tmpstr, _countof(tmpstr), filename);
    ...
    GetPrivateProfileStringW(L"led", L"chip_number", L"6710 ", tmpstr, _countof(tmpstr), filename);
...
HRESULT led1509306_hook_init(const struct led1509306_config *cfg)
{
    ...
    for (int i = 0; i < led1509306_nboards; i++)
    {
        ...
        dprintf("LED: initialized on COM%d\n", port);
    }
...
static HRESULT led1509306_handle_irp_locked(int board, struct irp *irp)
{
    ...
    for (;;) {
        ...
        if (FAILED(hr)) {
            dprintf("LED: Processing error: %x\n", (int) hr);
        }
    }
}
...
static HRESULT led1509306_req_dispatch(int board, const struct led1509306_req_any *req)
{
    switch (req->cmd) {
        ...
        case LED_15093_06_CMD_BOOTLOADER:
            return led1509306_req_enter_bootloader(board, req);

        default:
            dprintf("LED: Unhandled command %02x\n", req->cmd);
...
static HRESULT led1509306_req_reset(int board, const struct led1509306_req_any *req)
{
    dprintf("LED: Reset (board %u, type %02x)\n", board, req->payload[0]);

    if (req->payload[0] != 0xd9)
        dprintf("LED: Warning -- Unknown reset type %02x\n", req->payload[0]);
...
static HRESULT led1509306_req_get_board_info(int board)
{
    dprintf("LED: Get board info (board %u)\n", board);
...
static HRESULT led1509306_req_get_fw_sum(int board)
{
    dprintf("LED: Get firmware checksum (board %u)\n", board);
...
static HRESULT led1509306_req_get_protocol_ver(int board)
{
    dprintf("LED: Get protocol version (board %u)\n", board);
...
static HRESULT led1509306_req_get_board_status(int board)
{
    dprintf("LED: Get board status (board %u)\n", board);
...
static HRESULT led1509306_req_set_disable_response(int board, const struct led1509306_req_any *req)
{
    dprintf("LED: Disable LED responses (board %u)\n", board);
...
static HRESULT led1509306_req_set_timeout(int board, const struct led1509306_req_any *req)
{
    dprintf("LED: Set timeout (board %u)\n", board);
...
static HRESULT led1509306_req_enter_bootloader(int board, const struct led1509306_req_any *req) {
dprintf("LED: Enter bootloader??? (board %u)\n", board);
    ...
    resp.status = 1;
    resp.cmd = LED_15093_06_CMD_BOOTLOADER;
    resp.report = 1;
