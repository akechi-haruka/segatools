...
static HRESULT io4_async_poll(void *ctx, struct irp *irp)
{
	...
	now = GetTickCount();
	if (now - sta > 100){
		dprintf("USB I/O: Poll took %ld ms!!!\n", now - sta);
	}
	...
	hr = iobuf_write(&irp->read, &in, sizeof(in));

	now = GetTickCount();
	if (now - sta > 100){
		dprintf("USB I/O: iobuf_write took %ld ms!!!\n", now - sta);
	}
