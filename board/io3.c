...
static HRESULT io3_cmd_read_rotary(
        struct io3 *io3,
        struct const_iobuf *req_buf,
        struct iobuf *resp_buf);
        ...
        struct iobuf *resp)
{
    ...
    case JVS_CMD_READ_ROTARY:
        return io3_cmd_read_rotary(io3, req, resp);
        ...
        struct iobuf *resp_buf)
{
    ...
    if (io3->ops->read_rotary != NULL) {
        io3->ops->read_rotary(io3->ops_ctx, rotary, req.nrotary);
    }
