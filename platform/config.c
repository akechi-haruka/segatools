...
void netenv_config_load(struct netenv_config *cfg, const wchar_t *filename)
{
    ...
    cfg->force = GetPrivateProfileIntW(L"netenv", L"force", 0, filename);

    cfg->use_tracert_bypass = GetPrivateProfileIntW(L"netenv", L"use_tracert_bypass", 0, filename);
    cfg->full_addr = GetPrivateProfileIntW(L"netenv", L"full_addr", 0, filename);
    ...
    GetPrivateProfileStringW(
                ...
                _countof(ipstring),
                filename);
    swscanf(ipstring, L"%u.%u.%u.%u", &ip[0], &ip[1], &ip[2], &ip[3]);
    cfg->virtual_ipaddr = (ip[0] << 24) | (ip[1] << 16) | (ip[2] << 8) | (ip[3] << 0);
    ...
    GetPrivateProfileStringW(
                    ...
                    filename);
    swscanf(ipstring, L"%u.%u.%u.%u", &ip[0], &ip[1], &ip[2], &ip[3]);
    cfg->virtual_router = (ip[0] << 24) | (ip[1] << 16) | (ip[2] << 8) | (ip[3] << 0);
    ...
    GetPrivateProfileStringW(
                    ...
                    filename);
    swscanf(ipstring, L"%u.%u.%u.%u", &ip[0], &ip[1], &ip[2], &ip[3]);
    cfg->virtual_bcast = (ip[0] << 24) | (ip[1] << 16) | (ip[2] << 8) | (ip[3] << 0);
}
...
void nusec_config_load(struct nusec_config *cfg, const wchar_t *filename)
{
    ...
    cfg->skip_trace_erase = GetPrivateProfileIntW(L"keychip", L"skip_trace_erase", 0, filename);
    cfg->initial_nearfull = GetPrivateProfileIntW(L"keychip", L"initial_nearfull", 0x00010200, filename);
    cfg->initial_playlimit = GetPrivateProfileIntW(L"keychip", L"initial_playlimit", 1024, filename);
